...
def main():
    ...
    src_ids = " ".join("SRC_" + mmid(s) for s in {x for mp in mappings.values() for x in mp.sources})
    tgt_ids = " ".join("TGT_" + mmid(t) for t in {x for mp in mappings.values() for x in mp.targets})
